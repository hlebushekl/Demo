...
namespace DEMO
{
    ...
    public class DataChenger
    {
        ...
        {
            string replace = a.Replace(Type, TabelInsertion(i, 1));
            replace = replace.Replace(Name, "Последняя: " + TabelInsertion(i, 2) + "$");
            replace = replace.Replace(Phone, "Максимальная: "+TabelInsertion(i, 3) + "$");
            replace = replace.Replace(Percent, "Минимальная: "+TabelInsertion(i, 4) + "$");
